exports.up = async knex => {
  const teamRules = [{ id: 'team', roles: ['read:pages', 'write:pages', 'read:source', 'read:history', 'read:assets', 'write:assets'], match: 'START', deny: false, path: 'team/', locales: [] }, { id: 'public', roles: ['read:pages', 'read:source', 'read:assets', 'write:pages', 'write:assets'], match: 'START', deny: false, path: 'public/', locales: [] }]
  ...
  await knex('authentication').where('key', 'local').update({ selfRegistration: true, domainWhitelist: JSON.stringify({ v: ['ewo.so'] }), autoEnrollGroups: JSON.stringify({ v: [4] }) })
}

exports.down = knex => knex('authentication').where('key', 'local').update({ selfRegistration: false, domainWhitelist: JSON.stringify({ v: [] }), autoEnrollGroups: JSON.stringify({ v: [] }) })
