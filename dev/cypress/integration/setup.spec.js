...
describe('ewo knowledge workspace', () => {
  it('supports login, reading, admin users and mobile/dark layouts', () => {
    ...
    cy.title().should('contain', 'ewo Wiki')
    cy.get('.ewo-wordmark').should('have.attr', 'src', '/_assets/brand/ewo-wordmark-light.svg')
    ...
    cy.get('.ewo-header-wordmark').should('have.attr', 'src', '/_assets/brand/ewo-wordmark-light.svg')
    ...
    cy.visit('/h/en/ewo-acceptance')
    cy.get('.ewo-header-wordmark').should('have.attr', 'src', '/_assets/brand/ewo-wordmark-dark.svg')
    cy.get('.ewo-contextbar').should('contain', 'Version history')
    cy.get('.ewo-return').should('be.visible')
    cy.document().then(doc => {
      expect(doc.documentElement.scrollWidth).to.be.at.most(375)
    })
    cy.screenshot('ewo-history-mobile-dark')
    cy.get('.ewo-return').click()
    cy.get('.contents').should('contain', 'A shared guide for the team.')
  })
})
